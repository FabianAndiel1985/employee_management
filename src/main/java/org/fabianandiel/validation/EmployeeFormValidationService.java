...
public class EmployeeFormValidationService {
    ...
    /** validates the created person
     * @param createdPerson           the newly created person
     * @param createEmployeeErrorText where the error text goes to
     * @return true if the created person is valid, false if it is not
     */
    ...
    /**
     * validates the combination of subordinates and superios of created person
     *
    ...
    private static boolean validateSubordinatesAndSuperior(Set<Person> subordinates, Person superior, Set<Role> roles, Text createEmployeeErrorText) {
        ...
        if (roles.contains(Role.ADMIN) && (superior != null || subordinates.size() > 0)) {
            GUIService.setErrorText("Admins can not have superiors or subordinates", createEmployeeErrorText);
            return false;
        }
     ...
     *
     * @param personToValidate        person that  have to be validate
